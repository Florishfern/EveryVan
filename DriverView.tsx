...
export const DriverView: React.FC<DriverViewProps> = ({ activeNav }) => {
  ...
  const handleSimulateScan = () => {
    ...
    const b = bookings.find(item => item.id === selectedScanBookingId);
    if (b) {
      setScanResult(b);
      if (b.status === 'Paid') {
        setScanMessage({ text: '✓ ตั๋วผ่านการตรวจสอบแล้ว: ผู้โดยสารชำระเงินแล้ว สามารถขึ้นรถได้', type: 'success' });
      } else if (b.status === 'Boarded') {
        setScanMessage({ text: 'ℹ ตั๋วใบนี้ขึ้นรถเรียบร้อยแล้ว', type: 'info' });
      } else if (b.status === 'Pending Payment') {
        setScanMessage({ text: '⚠️ ผู้โดยสารยังไม่ได้ชำระเงินค่าตั๋ว กรุณาเรียกเก็บเงินสด 150 บาท หรือชำระผ่านระบบ', type: 'warning' });
      } else {
        setScanMessage({ text: '❌ ตั๋วไม่พร้อมใช้งานหรือถูกยกเลิกแล้ว', type: 'danger' });
      }
    }
  };

  const handleConfirmScannerPayment = async (bookingId: string) => {
    try {
      await confirmPayment(bookingId);
      setScanMessage({ text: '✓ ยืนยันชำระเงินและขึ้นรถเรียบร้อย!', type: 'success' });
      const updated = bookings.find(item => item.id === bookingId);
      if (updated) {
        setScanResult({ ...updated, status: 'Paid' });
        // Automatically board after payment
        await boardPassenger(bookingId);
      }
    } catch (err: any) {
      alert(err.message || 'เกิดข้อผิดพลาดในการยืนยันชำระเงิน');
    }
  };
      ...
      <div className="card grid-cols-2" style={{ gridColumn: 'span 2' }}>
        {activeVan ? (
          <div className="flex flex-col gap-4">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px' }}>
              ...
              <span className={`badge ${
                ...
                {activeVan.status === 'Waiting' ? 'กำลังรอคิว' : 
                 ...
                 activeVan.status === 'Departed' ? 'ออกจากสถานีแล้ว' : 'ขัดข้อง/อุบัติเหตุ'}
              </span>
            </div>
            ...
            {showAccidentForm && (
              <form onSubmit={handleSendAccidentReport} className="card flex flex-col gap-2" style={{ border: '2px solid var(--danger)', marginTop: '0' }}>
